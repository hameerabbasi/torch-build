if command -v nproc >/dev/null 2>&1; then
  # We used to set this based on how many physical cores were in the CPU, but this
  # causes compilation failures in containers with nproc < CPU cores.  nproc / 2 is
  # usually a safe guess.
  export MAX_JOBS=${MAX_JOBS:-$(($(nproc) / 2))}
fi

# Compilation type
export CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE:-Release}
# CMAKE_BUILD_TYPE=RelWithDebInfo gives you line numbers on gdb,
# but makes the symbol loading phase in gdb and the linking phase in compilation much slower.

# CUDA
if ! command -v nvidia-smi >/dev/null 2>&1; then
  export USE_CUDA=${USE_CUDA:-0}
else
  export USE_CUDA=${USE_CUDA:-1}
fi

# Currently we have no build envs that support ROCm, so disable it unconditionally to
# avoid issues compiling on desktops with AMD APUs.
export USE_ROCM=${USE_ROCM:-0}

# Faster linking
if command -v ld.mold >/dev/null 2>&1; then
  export CMAKE_LINKER_TYPE=MOLD
elif command -v ld.lld >/dev/null 2>&1; then
  export CMAKE_LINKER_TYPE=LLD
else
  export CMAKE_LINKER_TYPE=DEFAULT
fi

# Faster recompilation
export CCACHE_SLOPPINESS=${CCACHE_SLOPPINESS:-include_file_ctime,include_file_mtime,pch_defines,time_macros}
export USE_PRECOMPILED_HEADERS=${USE_PRECOMPILED_HEADERS:-1}
# Helps ccache still cache files between multiple worktrees
export USE_RELATIVE_PATHS=${USE_RELATIVE_PATHS:-1}

# Flags that mirror PyTorch defaults, but you may want to change
export USE_CUDNN=${USE_CUDNN:-$USE_CUDA} # CNNs
export USE_DISTRIBUTED=${USE_DISTRIBUTED:-1} # distributed
export USE_FBGEMM=${USE_FBGEMM:-1} # GEMMs
export USE_KINETO=${USE_KINETO:-1} # profiler

# Don't build what we don't need
export BUILD_AOT_INDUCTOR_TEST=${BUILD_AOT_INDUCTOR_TEST:-0} # C++ tests
export BUILD_TEST=${BUILD_TEST:-0} # C++ tests
export USE_PYTORCH_QNNPACK=${USE_PYTORCH_QNNPACK:-0} # mobile
export USE_XNNPACK=${USE_XNNPACK:-0} # mobile
# Disable these unless you are going to benchmark them
export USE_FLASH_ATTENTION=${USE_FLASH_ATTENTION:-0}
export USE_MEM_EFF_ATTENTION=${USE_MEM_EFF_ATTENTION:-0}

# cmake from conda
export CMAKE_PREFIX_PATH="$CONDA_PREFIX"

# ccache
export CMAKE_C_COMPILER_LAUNCHER=${CMAKE_C_COMPILER_LAUNCHER:-ccache}
export CMAKE_CXX_COMPILER_LAUNCHER=${CMAKE_CXX_COMPILER_LAUNCHER:-ccache}

if [[ $USE_CUDA -eq 1 ]]; then
  export CMAKE_CUDA_COMPILER=${CMAKE_CUDA_COMPILER:-"$CONDA_PREFIX/bin/nvcc"}
  export CMAKE_CUDA_COMPILER_LAUNCHER=${CMAKE_CUDA_COMPILER_LAUNCHER:-ccache}

  # Use cudatoolkit from conda (see pytorch-dev.yaml).  If you have it installed
  # system-wide (e.g. in qgpu) and want to use it, point CUDA_PATH and
  # CMAKE_CUDA_COMPILER to the correct folder.
  export CUDA_PATH="$CONDA_PREFIX"
  export CUDA_HOME="$CUDA_PATH"

  # Note: targets/x86_64-linux is added because of the use of deprecated
  # find_package(CUDA).  Usually `cuda_runtime.h` is found in CUDA_HOME and
  # find_package(CUDA) expects that.  However with conda, cuda headers are in
  # $CUDA_HOME/targets/x86_64-linux/include instead of $CUDA_HOME/include to
  # avoid clobbering the top level directory with names like mma.h.  The new way
  # (`enable_languages(CUDA)`) knows about this, but pytorch uses both
  # mechanisms.
  export CUDA_INC_PATH="$CUDA_PATH/targets/x86_64-linux/include"
fi

if [[ $OSTYPE =~ darwin ]]; then
    export CMAKE_OBJC_COMPILER_LAUNCHER=${CMAKE_OBJC_COMPILER_LAUNCHER:-ccache}
    export CMAKE_OBJCXX_COMPILER_LAUNCHER=${CMAKE_OBJCXX_COMPILER_LAUNCHER:-ccache}
fi
